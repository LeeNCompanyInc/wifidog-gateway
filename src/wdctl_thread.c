    ...
    if (pid > 0) {
        ...
        while (client) {
            ...
            safe_asprintf(&tempstring,
                          "CLIENT|ip=%s|mac=%s|token=%s|fw_connection_state=%u|fd=%d|counters_incoming=%llu|counters_outgoing=%llu|counters_last_updated=%lu|authenticated=%lu|sessiontimeout=%d|idletimeout=%d\n",
                          ...
                          client->counters.authenticated, client->counters.sessiontimeout, client->counters.idletimeout);
